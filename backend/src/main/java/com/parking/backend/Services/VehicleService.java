...
public class VehicleService {
    ...
    public Vehicle saveOrUpdateVehicle(Vehicle vehicle) {
        try {
            ...
            if (vehicle.getTypeVehicle() == TYPE_VEHICLE.BICYCLE) {
                if (vehicle.getLicencePlate() == null || vehicle.getLicencePlate().isEmpty() || vehicle.getLicencePlate().equals(" ")) {
   ...
   // Obtener un vehículo por placa
    ...
    private void validateVehicle(Vehicle vehicle) throws CustomException {
        ...
        //   String trimmedLicencePlate = vehicle.getLicencePlate().replaceAll("\\s+", "").toUpperCase();
      ...
      String trimmedLicencePlate="" ;
        if (vehicle.getTypeVehicle() == TYPE_VEHICLE.BICYCLE) {
            ...
            if (trimmedLicencePlate.length() < 8 || trimmedLicencePlate.length() > 14 || !trimmedLicencePlate.contains("-")) {
                throw new CustomException("La placa del vehículo tipo BICYCLE debe tener entre 8 y 13 caracteres  y contener -.");
            }
            // Validar que la patente comience con "BICYCLE-" y el resto sea alfanumérico
            if (!trimmedLicencePlate.startsWith("BICYCLE-") || !trimmedLicencePlate.substring(6).matches("[A-Z0-9-]+")) {
                throw new CustomException("La placa del vehículo tipo BICYCLE debe comenzar con 'BICYCLE-' seguido de 6 caracteres alfanuméricos.");
            }
            ...
            trimmedLicencePlate = vehicle.getLicencePlate().replaceAll("\\s+", "").toUpperCase();
            // Validar que la placa solo contenga letras y números para otros tipos de vehículos
            if (!trimmedLicencePlate.matches("[A-Z0-9]+")) {
                throw new CustomException("La placa del vehículo solo puede contener letras y números, sin caracteres especiales.");
            }
